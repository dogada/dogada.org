import { Layout } from "../core/Layout"

export const meta = {
  title: "D.V.D. homepage"
}

export default ({ children }) => <Layout meta={meta}>{children}</Layout>

# Dima "Dogada" Dogadailo

<img
  width="256"
  height="256"
  className="float-start mt-2 me-2"
  src="./static/dd_256.jpg"
  alt="photo"
/>

I create things mainly using Node.js, Typescript, React, SQL & NoSQL, Docker.

I'm interested in blockchain technologies, I believe that decentralization is the future and Bitcoin is the past.

One of my experimental projects Coect aims to create something like distributed Twitter (but without censorship) and Bitcoin (but with instant payments). You can find more details at: https://coect.dogada.org and https://fucr.dogada.org/.

I like meta-programming and prefer dynamic typing over static typing and believe that the best dynamic language isn't yet created. Once upon a time as an experiment, I even wrote a MetaJS compiler from LISP to Javascript that was able to guess a programmer's thoughts (more details at https://metajs.dogada.org).

I started with Java, then worked several years with Python/Django, but last years I prefer to use Javascript as the main programming language for both backend (NodeJs) and frontend (Angular, RiotJs, Svelte, React/Redux). On my Github you can find several javascript libraries for the NodeJs/React/Redux ecosystem.

My base project template (NextJs, Universal API, Postgres, Redis, Docker Swarm, PlayWright E2E testing) is available on GitHub: https://github.com/dogada/concepts.

After russia's invasion into Ukraine became intersted in EPS32 and low-latencey video processing on microcontrollers. Got some interestig results.

If you have any questions, please contact me for additional details.

### Projects

- [sifolks.com](https://sifolks.com) — A global cybernetic loop: people's reactions to important events lead to changes in government and society.
- [wikiket.com](https://wikiket.com) — Buyer-focused marketplace (mix of Wikipedia and Amazon). No ads, honest reviews of FPV drones, smartbands and other electronics.
- [CASP](/casp) — content-addressable publishing (<abbr title="Work In Progress">WIP</abbr>).
- [MultiSync](/multisync) — file-based event log sync for local-first apps (<abbr title="Work In Progress">WIP</abbr>).
- [Piki](/piki) — ActivityPub-federated wiki & note-taking local-first app (<abbr title="Work In Progress">WIP</abbr>).
- [Coect](https://coect.dogada.org/) — Decentralized media platform (blockchain and IPFS). It will fix Twitter sometime (<abbr title="Work In Progress">WIP</abbr>).

### Contacts

- Github: [github.com/dogada](https://github.com/dogada)
- X/Twitter: [x.com/d0gada](https://twitter.com/d0gada)
- Email: dvd at dogada.org
